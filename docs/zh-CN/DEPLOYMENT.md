...
这样不会污染系统目录。它会把可执行文件、README、更新日志、文档、图标、桌面元数据、插件目录和内置 数据服务 索引安装到 `/tmp/media-hit-install` 下。
...
本地测试不需要真实密钥。没有配置 访问密钥 时，软件使用安全示例采集。真实 访问密钥 只能保存在本地设置或环境变量中，不能提交到仓库。
