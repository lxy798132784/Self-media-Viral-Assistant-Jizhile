...
```text
QML 界面
仪表盘 | 内容库 | 接口库 | 爆文 API | 报告 | 选题 | 插件 | 设置
        |
        v
AppController
        |
        +-- ConfigManager       本地配置
        +-- ApiCatalog          内置接口索引
        +-- JizhiliaClient      请求体、HTTP、解析、重试、回退
...
| `ApiCatalog` | 定位和解析仓库内置极致了 endpoint 索引。 |
| `JizhiliaClient` | 构造请求体、执行 HTTP、解析 JSON、分类错误、重试和回退。 |
...
endpoint 索引放在 `vendor/jizhilia-api-knowledge/api-index.json`。测试和运行时优先使用仓库相对路径，其次使用编译期源码目录，再使用安装后的 data 路径。这样可以避免 GitHub Actions 因本机绝对路径不存在而失败。
