...
| 数据路径库 | 使用仓库内置内容数据服务 索引，支持分类筛选、data path 详情和按 data path 采集。 |
| 公众号爆文 | 独立页面覆盖 `key`、`keyword`、`pub_type`、`category`、`page`、`start_time`、`end_time` 参数控件。 |
...
| 设置 | 配置 访问密钥、验证码、data path、采集频率、采集次数、QPS、导出目录、任务和运行历史。 |
...
本地测试不需要真实密钥。没有配置 访问密钥 时，软件会使用安全示例采集，保证完整流程可以验证且不消耗数据路径额度。

可配置项包括：内容数据服务 Key、验证码、data path 路径、公众号爆文 参数、界面语言、采集频率、最大运行次数、QPS 限速和导出目录。
