...
ApplicationWindow {
    ...
    function help(key) {
        ...
        var en = {
            ...
            help_dashboard: "Dashboard: review article count, reads, and likes. Load samples, run the full self-test, collect by keyword, browse endpoints, and run the first endpoint. Click stat cards or the guide panel to write a status receipt, proving displayed surfaces are interactive.",
                ...
                ColumnLayout { anchors.fill: parent; anchors.margins: 24; spacing: 18
                    ...
                    Rectangle { Layout.fillWidth: true; Layout.fillHeight: true; color: panel; radius: 18; border.color: "#334155"
                        ColumnLayout { anchors.fill: parent; anchors.margins: 18; spacing: 16
                            ...
                            RowLayout {
                                Layout.fillWidth: true
                                TextField { id: endpointFilter; Layout.fillWidth: true; placeholderText: appController.language === "en" ? "Endpoint category filter" : "数据端点分类筛选"; text: appController.language === "en" ? "official account" : "公众号"; ToolTip.visible: hovered; ToolTip.text: appController.language === "en" ? "Filter bundled endpoint catalog by category" : "按分类筛选内置数据端点目录" }
                                Button { text: appController.language === "en" ? "Browse endpoints" : "浏览端点"; ToolTip.visible: hovered; ToolTip.text: appController.language === "en" ? "Show matching endpoint rows in the detail panel" : "在详情区显示匹配的数据端点"; onClicked: root.detailText = appController.apiEndpointRows(endpointFilter.text).join("\n") }
                                Button { text: appController.language === "en" ? "Run first endpoint" : "运行首个端点"; ToolTip.visible: hovered; ToolTip.text: appController.language === "en" ? "Run the first matched endpoint with the dashboard keyword" : "用仪表盘关键词运行首个匹配端点"; onClicked: { const row = appController.apiEndpointRows(endpointFilter.text)[0]; root.detailText = appController.endpointPathFromRow(row); appController.runEndpointRow(row, dashKeyword.text); } }
                            }
                    ...
                    Rectangle { Layout.fillWidth: true; Layout.fillHeight: true; color: panel; radius: 18; border.color: "#334155"
                        ColumnLayout { anchors.fill: parent; anchors.margins: 18; spacing: 10
                            ...
                            RowLayout { Layout.fillWidth: true
                                ...
                                Button { text: appController.language === "en" ? "Smoke plan" : "联调计划"; ToolTip.visible: hovered; ToolTip.text: appController.language === "en" ? "Show a safe preview-only troubleshooting plan" : "显示安全的预览式联调计划"; onClicked: hotPreview.text = appController.hotTypicalSmokePreview(hotKey.text, hotKeyword.text, hotPubType.currentValue, hotCategory.currentValue, hotPage.value, hotStart.text, hotEnd.text) }
                                Button { text: root.t("collect_hot"); ToolTip.visible: hovered; ToolTip.text: appController.language === "en" ? "Collect hot articles using the current parameters" : "按当前参数采集爆文"; onClicked: { appController.runHotTypicalCollection(hotKey.text, hotKeyword.text, hotPubType.currentValue, hotCategory.currentValue, hotPage.value, hotStart.text, hotEnd.text); hotPreview.text = appController.hotTypicalPayloadPreview(hotKey.text, hotKeyword.text, hotPubType.currentValue, hotCategory.currentValue, hotPage.value, hotStart.text, hotEnd.text); } }
                            }
                            Label { text: appController.language === "en" ? "Parameter coverage" : "参数覆盖"; color: textSub }
                            ListView { Layout.fillWidth: true; Layout.preferredHeight: 100; model: appController.hotTypicalParameterRows(); delegate: Rectangle { width: ListView.view.width; height: 25; color: hotParamMouse.containsMouse ? "#1e293b" : "transparent"; Text { anchors.verticalCenter: parent.verticalCenter; text: modelData; color: textMain; elide: Text.ElideRight; width: parent.width } MouseArea { id: hotParamMouse; anchors.fill: parent; hoverEnabled: true; cursorShape: Qt.PointingHandCursor; onClicked: appController.noteSelection(appController.language === "en" ? "API parameter" : "接口参数", modelData) } } }
                            TextArea { id: hotPreview; Layout.fillWidth: true; Layout.fillHeight: true; readOnly: true; wrapMode: TextArea.Wrap; text: appController.hotTypicalPayloadPreview("[empty]", hotKeyword.text, hotPubType.currentValue, hotCategory.currentValue, hotPage.value, hotStart.text, hotEnd.text); color: textMain; ToolTip.visible: hovered; ToolTip.text: appController.language === "en" ? "Preview output for parameters or safe collection plan" : "参数或安全联调计划的预览输出"; background: Rectangle { color: panel2; radius: 12; border.color: "#334155" } }
                        }
                    }
                }
            }
